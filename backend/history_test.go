...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Order{Customer: "Meena", Garment: "blouse", DueDate: "2026-08-25"})
	saved, err := store.Save(Record{Input: in, Headline: 0, Label: "pending"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "pending" {
